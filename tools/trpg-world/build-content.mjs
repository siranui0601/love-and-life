...
import {DEFAULT_PROCESSES,DEFAULT_PROCESS_STRUCTURES,DEFAULT_SHIPMENTS,DEFAULT_ACTOR_OPERATIONS,DEFAULT_OCCUPANCY_CLAIMS} from '../../src/shared/trpg-world/process-content.js';
...
items.push({id:'igniter',name:'火付け用の小箱',kind:'tool',price:6},{id:'tainted-dose',name:'封の割れた薬包',kind:'material',price:1,saleOnly:true});
...
for(const operation of DEFAULT_ACTOR_OPERATIONS){const actor=npcs.find(n=>n.id===operation.actorId),item=operation.restraintItemId||operation.weaponItemId;if(actor)actor.possessions={...actor.possessions,...operation.requiredResources,...(item?{[item]:1}:{})};}
...
const lord=npcs.find(n=>n.id==='NPC009');if(lord)lord.appointments=[{siteId:'LOC_TRADE_LORD_MANOR',startsAt:2*86400+7*3600,endsAt:2*86400+13*3600,activity:'屋敷で診察と朝の用務を受ける'}];
...
for(const claim of DEFAULT_OCCUPANCY_CLAIMS)for(const id of claim.residentIds){const actor=npcs.find(n=>n.id===id);if(actor){actor.appointments||=[];actor.appointments.push({siteId:claim.facilityId,startsAt:claim.departAt-3600,endsAt:claim.departAt+4*3600,activity:'住居で届けられる書類を受け取るため待ち合わせている'});}}
const structures=structuredClone([...DEFAULT_STRUCTURES,...DEFAULT_PROCESS_STRUCTURES]);structures.find(s=>s.id==='grain-store').causeOperations=['grain-ignition'];
const content={version:1,revision:'pending',time:{days:10,scale:60,startSeconds:25200},regions,routes,npcs,events,causalScenarios:DEFAULT_CAUSAL_SCENARIOS,structures,processes,shipments:DEFAULT_SHIPMENTS,actorOperations:DEFAULT_ACTOR_OPERATIONS,occupancyClaims:DEFAULT_OCCUPANCY_CLAIMS,skills,items,recipes,jobs,...combat,
