...
const record=JSON.parse(await fs.readFile(new URL('mine-replay.json',root),'utf8'));
const split=JSON.parse(await fs.readFile(new URL('mine-intermediate.json',root),'utf8'));
...
const result={status:'PASS',sourceOperation:split.operation,sourceHash:digest(split.state),contentRevision:c.revision,
...
await fs.writeFile(new URL('save-summary.json',root),JSON.stringify(result,null,2));console.log(JSON.stringify(result));
