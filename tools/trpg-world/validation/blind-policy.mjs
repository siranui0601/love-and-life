...
export function chooseBlind(o,m){
 ...
 const job=actions.find(a=>!m.attempted.includes(a.key)&&(
 ...
 if(job){if(job.available){m.attempted.push(job.key);m.need=null;return act(job);}m.need=job.requirements;}
 if(m.need){
  ...
  if(item||skill){const type=item?'buy':'train',key=item?'itemId':'skillId',id=item?item[0]:skill;
   ...
   const unexplored=o.places.find(t=>(item?['shop','stable']:['trainer']).includes(t.kind)&&!o.services.some(s=>s.id===t.id));if(unexplored)return walk(unexplored);
