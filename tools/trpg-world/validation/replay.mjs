...
export function meaningfulState(state) {
 ...
 return canonical({knowledge:state.knowledge.map(knowledgeMeaning).sort((a,b)=>a.id.localeCompare(b.id,'en')),inventory:state.player.inventory,gold:state.player.gold,
  ...
  properties:state.properties,facilities:state.facilities,processes:state.processes,structures:state.structures,personCustodies:state.personCustodies,actorOperations:state.actorOperations,duties:state.duties,shipments:state.shipments,careCases:state.careCases,deviceTransfers:state.deviceTransfers,occupancyClaims:state.occupancyClaims});
}
