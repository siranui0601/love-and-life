...
export function optionsFromView(view) {
 ...
 return [...view.interactables.flatMap(target=>target.actions.filter(o=>o.available!==false).map(option=>({signature:semanticIdentity(option,target.id),label:option.label,targetName:target.name,requirements:option.requirements,
  command:commandForOption(option,target.id),progress:option.type==='process'&&!option.id.endsWith('/inspect')||option.type==='maintain'||option.type==='causal'&&!['escort','release'].includes(option.id)||option.id==='inspect'&&target.kind==='evidence'}))),
...
export function meaningfulState(state) {
 ...
 return canonical({knowledge:state.knowledge.map(knowledgeMeaning).sort((a,b)=>a.id.localeCompare(b.id,'en')),inventory:state.player.inventory,gold:state.player.gold,
  ...
  properties:state.properties,facilities:state.facilities,processes:state.processes,structures:state.structures,personCustodies:state.personCustodies,actorOperations:state.actorOperations,duties:state.duties,shipments:state.shipments,careCases:state.careCases,deviceTransfers:state.deviceTransfers});
}
