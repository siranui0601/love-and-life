...
test("作者用の人徳縦工程はT01からT19を重複なく含む", () => {
  ...
  assert.deepEqual([...new Set(ids)].sort(), Array.from({ length: 19 }, (_, index) => `T${String(index + 1).padStart(2, "0")}`));
});
...
test("T03解決後に生活・家族・移動の異なる短い三択を表示する", () => {
  ...
  assert.deepEqual(actions.map((action) => action.label), ["巡礼便に加わる", "失踪者の家族を訪ねる", "村の見張りを引き継ぐ"]);
...
test("巡礼便の中心枝はNPCを停止させず三つの自主GOAPを残して神殿へ進む", () => {
  ...
  assert.equal(state.playerState.goapRequests["GOAP-T04-JILL-PILGRIM-ROAD"].destination, "古代神殿");
  assert.equal(state.playerState.goapRequests["GOAP-T04-FINN-RETURN-MAP"].destination, "田園の村");
...
test("門前の三択は生活・物流・観察で異なる証拠と次施設を作る", () => {
  ...
  assert.deepEqual(authoredMissionFlowExclusiveActions(rolls).map((action) => action.label), ["名簿を管理棟へ運ぶ", "巡礼者へ水を配る", "鈴と荷車を数える"]);
...
test("ファルコへの短い三択が既存T04聞き取りへ正式接続する", () => {
  ...
  const actions = authoredMissionFlowExclusiveActions(state, context);
  assert.deepEqual(actions.map((action) => action.label), ["列順を聞く", "隠蔽を問いただす", "Day1の異常を聞く"]);
  assert.ok(actions.every((action) => action.actionId === action.id));

  const selected = choose(state, "列順を聞く", context);
  assert.equal(state.playerState.missions["MSN-T04"].progress.hear, 1);
  assert.equal(state.authoredMissionFlows[relay.FLOW_ID].openingChoiceId, "disappearance_pattern");
  ...
  assert.ok(!authoredMissionFlowExclusiveActions(state, context)?.some((action) => action.authoredHumanT04FalcoProxy));
});
...
test("家族枝は正本のエイダ日記証拠を保存し、死亡NPCを表示しない", () => {
  ...
  assert.ok(state.authoredMissionFlows[relay.FLOW_ID].evidenceIds.includes("T04-EVIDENCE-EDA-DIARY-GLYPH"));
  assert.ok(state.playerState.missions["MSN-T04"].discoveries.some((entry) => entry.id === "T04-EVIDENCE-EDA-DIARY-GLYPH"));

  const dead = runtime();
  dead.livingWorld.npcStates.NPC060.lifeStatus = "dead";
  assert.equal(relay.initialEligible(dead), false);
  assert.ok(!authoredMissionFlowExclusiveActions(dead)?.some((action) => action.authoredHumanT04RelayChoice));
});
...
test("選択済みsceneと閉じた枝は保存復元後も復活しない", () => {
  ...
  assert.ok(!authoredMissionFlowExclusiveActions(saved, { presentNpcs: [{ id: "NPC053" }] }).some((action) => action.relayScene === "village"));
});