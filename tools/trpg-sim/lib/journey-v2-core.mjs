...
export function syncClockV2(state) { Object.assign(state, clockFromMinute(state.absoluteMinute)); state.daypart = state.minuteOfDay < 480 ? "dawn" : state.minuteOfDay < 1080 ? "day" : state.minuteOfDay < 1320 ? "dusk" : "night"; }

export function publishRumorV2(state, data) {
  const id = data.id ?? `RUM2-${String(state.rumors.length + 1).padStart(5, "0")}`; if (state.rumorById[id]) return state.rumorById[id];
  ...
  state.rumors.push(rumor); state.rumorById[id] = rumor; if (data.playerKnows !== false) state.player.knownRumorIds.add(id);
  state.history.push({ type: "RUMOR_PUBLISHED", minute: state.absoluteMinute, rumorId: id, troubleId: rumor.troubleId }); return rumor;
}
...
function consequences(state, id, status) {
  ...
  if (id === "T13" && failed) Object.assign(state.worldFlags, { worldTreeFallen: true, forestMazeOpen: true }); if (id === "T08" && status === "resolved") state.worldFlags.elfApproval = true; if (id === "T12" && status === "resolved") state.worldFlags.blackridgePermit = true;
  if (id === "T15" && failed) state.worldFlags.foreignFleetLanded = true; if (id === "T17" && status === "resolved") state.worldFlags.secondSummoningStopped = true; if (id === "T18" && failed) state.worldFlags.colossusAwake = true;
  if (id === "T19" && status === "critical") state.worldFlags.blackridgeArmyAtCapital = true; if (id === "T19" && status === "failed") state.worldFlags.capitalFallen = true; if (before !== JSON.stringify(state.worldFlags)) state.routeCache = {};
}

export function transitionTroubleV2(state, model, id, to, reason) {
  const runtime = state.troubles[id]; if (!runtime || runtime.status === to || TERMINAL.has(runtime.status)) return false; const from = runtime.status; runtime.status = to; runtime.transitions.push({ from, to, minute: state.absoluteMinute, reason });
  if (to === "active") runtime.activatedAt = state.absoluteMinute; if (TERMINAL.has(to)) runtime.terminalAt = state.absoluteMinute; consequences(state, id, to); const trouble = model.troubleById[id];
...
export function updateTroublesV2(state, model) {
  for (const trouble of model.troubles) { const runtime = state.troubles[trouble.id]; if (runtime.status === "scheduled" && state.absoluteMinute >= momentV2(trouble.startDay, trouble.startPhase)) { const open = gateOpen(state, trouble.id); transitionTroubleV2(state, model, trouble.id, open ? "active" : "suppressed", open ? "scheduled-onset" : "gate-closed"); }
...
export function propagateRumorsV2(state, model) {
  for (const rumor of state.rumors) for (const npc of model.npcs) { if (rumor.recipients[npc.id]) continue; const hub = state.day <= 1 ? npc.initialLocation : npc.home || npc.initialLocation; const hours = shortestTravelPlan(model, state, rumor.origin, hub)?.hours;
    if (!Number.isFinite(hours) || state.absoluteMinute < rumor.originMinute + Math.ceil(hours * 60) + 30) continue; if (!(rumor.importance >= .8 || npc.relatedTroubleIds.includes(rumor.troubleId))) continue;
    rumor.recipients[npc.id] = { minute: state.absoluteMinute, hub }; incV2(state.progress, "rumors.npcRecipients"); if (rumor.troubleId && npc.relatedTroubleIds.includes(rumor.troubleId)) { incV2(state.progress, "rumors.npcReplans"); if (npc.disposition === "mitigate") state.troubles[rumor.troubleId].casualtyMitigation += npc.importanceWeight * .05; } }
}
...
export function routeTagsV2(model, routeIds) { const text = routeIds.map((id) => `${model.routeById[id]?.nature ?? ""} ${model.routeById[id]?.from ?? ""} ${model.routeById[id]?.to ?? ""}`).join(" "); return [/森|林/u.test(text) && "forest", /荒野|街道|近郊|平原/u.test(text) && "wilderness", /洞窟|坑道/u.test(text) && "cave", /川|河|水|港|海/u.test(text) && "waterSide"].filter(Boolean); }
