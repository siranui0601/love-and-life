...
const SAFE_RETURN_FACILITY_PATTERN = /門|入口|広場|駅|港|船着|宿|詰所/u;
...
function exactResourceScaledBuild(prepared, state) {
  ...
  return {
    ...
    // Checkpoint D resource continuity: current resources and maxima are two
    // different facts.  Never shrink maxHp/maxMp to represent an injured or
    // exhausted player, and do not apply the historical 18%/3% entry floors.
    maxHp,
    maxMp,
    resourceMaxHp: maxHp,
    resourceMaxMp: maxMp,
...
function canonicalDefeatReturnPoint(state, model) {
  const location = state?.player?.location ?? null;
  const facilities = location ? (model?.facilitiesByHub?.[location] ?? []) : [];
  const current = facilities.find((facility) => facility.id === state?.player?.facilityId) ?? null;
  const isSafe = (facility) => Boolean(facility && SAFE_RETURN_FACILITY_PATTERN.test(`${facility.name ?? ''} ${facility.type ?? ''}`));
  const facility = (isSafe(current) ? current : null)
    ?? facilities.find(isSafe)
    ?? current
    ?? facilities[0]
    ?? null;
  return { location, facilityId: facility?.id ?? state?.player?.facilityId ?? null };
}

function applyCanonicalDefeatReturn(state, continuation) {
  const point = continuation?.defeatReturn;
  if (!point?.location) return;
  const fromLocation = state.player.location;
  const fromFacilityId = state.player.facilityId ?? null;
  state.player.location = point.location;
  state.player.facilityId = point.facilityId ?? null;
  state.history.push({
    type: 'BATTLE_DEFEAT_RETURN',
...
export function beginInteractiveBattleAction(state, model, data, skills, catalog, profileInput, action) {
  ...
  const defeatReturn=canonicalDefeatReturnPoint(state,model);
  ...
  output.continuation.defeatReturn=defeatReturn;
...
export function settleInteractiveBattleAction(state, model, data, skills, catalog, profileInput, continuation, battleResult) {
  ...
  else if(output?.battle&&!output.battle.won&&!output.battle.fled){applyCanonicalDefeatReturn(state,continuation);state.history.push({type:'BATTLE_DEFEAT_RECOVERY',minute:state.absoluteMinute,hpRatio:state.player.hpRatio,mpRatio:state.player.mpRatio,gold:state.player.gold});}
