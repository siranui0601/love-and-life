export function renderPlayerSimulationMarkdownV2(report) {
  const rows = report.tuned.profiles.map((entry) =>
    `| ${entry.label} | ${entry.level.median} | ${entry.missionsCompleted.median} | ${entry.specialMissionsCompleted.median} | ${entry.resolvedTroubles.median} | ${entry.failedTroubles.median} | ${entry.localMovementActions.median} | ${entry.regionalMovementActions.median} | ${entry.battles.median} | ${(entry.winRate.median * 100).toFixed(1)}% | ${entry.learnedSkills.median} | ${entry.flagSkillsLearned.median} |`
  ).join("\n");
...
| 方針 | Lv | 全任務 | 特別任務 | 解決T | 失敗T | 地域内移動 | 地域外移動 | 戦闘 | 勝率 | 習得技能 | 条件技能 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
${rows}
