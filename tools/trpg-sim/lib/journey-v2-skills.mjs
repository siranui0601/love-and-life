...
function refreshKey(state) {
  return JSON.stringify({
    level: state.player.level, sp: state.player.sp, gold: state.player.gold,
    hp: +state.player.hpRatio.toFixed(4), mp: +state.player.mpRatio.toFixed(4),
    location: state.player.location, facilityId: state.player.facilityId,
    equipment: state.player.equipment, skills: state.player.skills,
    items: state.player.inventory.items, progress: state.progress,
...
function containsBattleScope(conditions) { return flattenConditionLeaves(conditions).some((leaf) => BATTLE_SCOPES.has(leaf.scope)); }
function mark(state, setName, mapName, skill, type) { const set = state.skillState[setName]; if (set.has(skill.id)) return false; set.add(skill.id); state.skillState[mapName][skill.id] = state.absoluteMinute; state.history.push({ type, minute: state.absoluteMinute, skillId: skill.id, acquisitionCode: skill.acquisitionCode }); return true; }
...
export function refreshSkillStateV2(state, data, skills, profile) {
  const beforeKey = refreshKey(state);
  if (state.skillState.lastRefreshKey === beforeKey) return state.skillState.effective;
  ...
  for (const skill of skills) {
    ...
    if (code.includes("equipment") && context.equipment.grantedSkillIds.has(skill.id)) {
      ...
      if (!state.skillState.equipmentEverActive.has(skill.id)) { state.skillState.equipmentEverActive.add(skill.id); state.metrics.equipmentGrantActivations += 1; state.history.push({ type: "SKILL_EQUIPMENT_ACTIVE", minute: state.absoluteMinute, skillId: skill.id }); }
    }
    if (AUTO_CODES.has(code) && prerequisitesMet && learnedConditions && !state.player.skills.has(skill.id)) {
      state.player.skills.add(skill.id); state.metrics.skillsLearnedByCode[code] = Number(state.metrics.skillsLearnedByCode[code] ?? 0) + 1; state.history.push({ type: "SKILL_GRANTED_AUTOMATIC", minute: state.absoluteMinute, skillId: skill.id, acquisitionCode: code });
    }
    const eventEligible = code.includes("event") && !state.player.skills.has(skill.id) && prerequisitesMet && (grantConditions || flagUnlocked && learnedConditions);
    if (eventEligible) {
      state.player.skills.add(skill.id); state.progress.events.grantedSkillIds.add(skill.id); state.metrics.eventSkillGrants += 1; state.metrics.skillsLearnedByCode[code] = Number(state.metrics.skillsLearnedByCode[code] ?? 0) + 1; state.history.push({ type: "SKILL_GRANTED_EVENT", minute: state.absoluteMinute, skillId: skill.id, acquisitionCode: code });
    }
  }
  ...
  while (state.player.sp > 0) {
    ...
    const candidates = skills.filter((skill) => LEVEL_CODES.has(skill.acquisitionCode)).filter((skill) => !state.player.skills.has(skill.id)).filter((skill) => Number(skill.spCost ?? 0) > 0 && Number(skill.spCost ?? 0) <= state.player.sp).filter((skill) => prerequisitesOf(skill).every((id) => state.player.skills.has(id))).filter((skill) => evaluateConditions(skill.learnConditions, context)).filter((skill) => skill.acquisitionCode !== "flag_unlocked" || evaluateConditions(skill.eventUnlockConditions, context)).sort((left, right) => skillScore(right, profile) - skillScore(left, profile) || left.id.localeCompare(right.id));
    ...
    state.player.skills.add(selected.id); state.player.sp -= Number(selected.spCost ?? 0); state.metrics.skillsLearnedByCode[selected.acquisitionCode] = Number(state.metrics.skillsLearnedByCode[selected.acquisitionCode] ?? 0) + 1;
...
export function battleSkillIdsV2(state, data) {
  ...
  return [...effectiveSkillIdsV2(state, data)].filter((id) => { const skill = data.playerSkillById.get(id); if (!skill || ["non_skill", "deleted"].includes(skill.acquisitionCode)) return false; if (containsBattleScope(skill.activationConditions)) return true; return evaluateConditions(skill.activationConditions, context); });
}

export function auditSkillCatalogV2(skills, data) {
  const ids = new Set(skills.map((skill) => skill.id)); const missingPrerequisites = []; const equipmentGrantMissing = [];
  ...
  const byCode = {}; for (const skill of skills) byCode[skill.acquisitionCode ?? "unknown"] = Number(byCode[skill.acquisitionCode ?? "unknown"] ?? 0) + 1;
