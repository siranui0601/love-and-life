...
function aggregate(profile, runs) {
  ...
  return {
    ...
    visitedHubs: field("visitedHubs"), visitedFacilities: field("visitedFacilities"), movementBlocked: summaries.reduce((sum, x) => sum + x.movementBlocked, 0), movementObjectivesWaiting: field("movementObjectivesWaiting"), travelInterrupted: field("travelInterrupted"), travelEncountersAvoided: field("travelEncountersAvoided"),
...
function findings(report, targets) {
  ...
  add(balanced.winRate.median >= targets.balancedWinRateMin && balanced.winRate.median <= targets.balancedWinRateMax ? "verified" : "warning", "BATTLE_BALANCE", `均衡型勝率 ${(balanced.winRate.median * 100).toFixed(1)}% / 回避遭遇 ${balanced.travelEncountersAvoided.median}`);
