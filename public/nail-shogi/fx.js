...
async function growOne(event, ctx) {
  ...
  const pulse = svgEl("circle", { class: "fx-growth-pulse", cx: a.x, cy: a.y, r: 13 });
  ctx.fxSvg.append(line, pulse);
  if (reduced()) { await sleep(45); line.remove(); pulse.remove(); return; }
  const length = Math.hypot(b.x - a.x, b.y - a.y) || 100;
  line.style.strokeDasharray = `${length}`;
  line.style.strokeDashoffset = `${length}`;
  const move = line.animate([
    { strokeDashoffset: length, opacity: .35 },
    { strokeDashoffset: 0, opacity: 1 },
  ], { duration: event.type === "sculpt-grow" ? 155 : 205, easing: "cubic-bezier(.2,.8,.25,1)" });
  const pop = pulse.animate([
    { transformOrigin:`${a.x}px ${a.y}px`, transform:"scale(.6)", opacity:.75 },
    { transformOrigin:`${a.x}px ${a.y}px`, transform:"scale(1.45)", opacity:0 },
  ], { duration:180, easing:"ease-out" });
  await Promise.allSettled([move.finished, pop.finished]);
  line.remove(); pulse.remove();
}
...
async function hookSwipe(event, ctx) {
  ...
  const claw = svgEl("path", {
    class: "fx-hook-claw",
    d: `M ${p.x-78*side} ${p.y-15} L ${p.x-14*side} ${p.y-15} C ${p.x+18*side} ${p.y-15} ${p.x+38*side} ${p.y+2} ${p.x+32*side} ${p.y+27} C ${p.x+26*side} ${p.y+51} ${p.x-2*side} ${p.y+59} ${p.x-21*side} ${p.y+43} L ${p.x-7*side} ${p.y+26} C ${p.x+2*side} ${p.y+34} ${p.x+11*side} ${p.y+29} ${p.x+13*side} ${p.y+19} C ${p.x+15*side} ${p.y+8} ${p.x+5*side} ${p.y+3} ${p.x-10*side} ${p.y+3} L ${p.x-78*side} ${p.y+3} Z`,
  });
  ...
  const ring = svgEl("circle", { class:"fx-hook-ring", cx:p.x, cy:p.y, r:28 });
  ctx.fxSvg.append(claw, ring, slash);
  navigator.vibrate?.([15, 14, 34]);
  if (reduced()) {
    await sleep(110);
    claw.remove(); slash.remove(); ring.remove();
    return;
  }
  const a = claw.animate([
    { transform:`translate(${-42*side}px,0) rotate(${side*-8}deg)`, opacity:0 },
    { transform:"translate(0,0) rotate(0deg)", opacity:1, offset:.48 },
    { transform:`translate(${24*side}px,8px) rotate(${side*8}deg)`, opacity:0 },
  ], { duration:390, easing:"cubic-bezier(.18,.88,.25,1)" });
  const b = slash.animate([
    { opacity:0, transformOrigin:`${p.x}px ${p.y}px`, transform:"scale(.2)" },
    { opacity:1, transformOrigin:`${p.x}px ${p.y}px`, transform:"scale(1.1)", offset:.42 },
    { opacity:0, transformOrigin:`${p.x}px ${p.y}px`, transform:"scale(1.35)" },
  ], { duration:270, delay:115, easing:"ease-out" });
  const c = ring.animate([
    { opacity:.8, transformOrigin:`${p.x}px ${p.y}px`, transform:"scale(.5)" },
    { opacity:0, transformOrigin:`${p.x}px ${p.y}px`, transform:"scale(2)" },
  ], { duration:300, delay:75, easing:"ease-out" });
  boardNudge(ctx.boardWrap, 1.35);
  await Promise.allSettled([a.finished,b.finished,c.finished]);
  claw.remove(); slash.remove(); ring.remove();
}
...
function capturePieces(points, ctx, className, bias = 1) {
  if (reduced()) return [];
  const source = points.slice(-5);
  return source.map((item, i) => {
    const p = viewPoint(item.cell, ctx.viewPlayer);
    const piece = svgEl("rect", { class:className, x:p.x-15, y:p.y-22, width:30, height:44, rx:13 });
    ctx.fxSvg.append(piece);
    const side = i % 2 ? -1 : 1;
    const dx = side * (48 + i*14) * bias;
    const dy = -38 - i*13;
    const a = piece.animate([
      { transformOrigin:`${p.x}px ${p.y}px`, transform:"translate(0,0) rotate(0deg) scale(1)", opacity:1 },
      { transformOrigin:`${p.x}px ${p.y}px`, transform:`translate(${dx*.45}px,${dy}px) rotate(${side*24}deg) scale(1.05)`, opacity:1, offset:.42 },
      { transformOrigin:`${p.x}px ${p.y}px`, transform:`translate(${dx}px,${55+i*12}px) rotate(${side*(95+i*18)}deg) scale(.84)`, opacity:0 },
    ], { duration:440+i*30, delay:70+i*22, easing:"cubic-bezier(.2,.75,.3,1)" });
    return a.finished.finally(()=>piece.remove());
  });
}
...
async function captureOne(event, ctx) {
  ...
  const widths = [78,58,62,57,48];
  const heights = [72,88,100,90,70];
  const w = widths[event.defenderFinger] || 58;
  const h = heights[event.defenderFinger] || 88;
  const finger = svgEl("rect", { class: "fx-capture-finger", x: target.x - w/2, y: target.y - h/2, width:w, height:h, rx:Math.min(30,w*.42), fill: event.defenderPlayer === 0 ? "#f2ad87" : "#e3bc94" });
  const nailBed = svgEl("rect", { class: "fx-capture-bed", x: target.x - 16, y: target.y - 26, width:32, height:42, rx:14 });
  const burst = svgEl("circle", { class:"fx-capture-burst", cx:target.x, cy:target.y, r:24 });
  ctx.fxSvg.append(attackLine);
  if (defendLine) ctx.fxSvg.append(defendLine);
  ctx.fxSvg.append(finger,nailBed,burst);
  const word = document.createElement("div");
  word.className = "capture-word";
  word.textContent = "詰み！";
  ctx.fxLayer.append(word);
  navigator.vibrate?.([24,22,48]);
  if (reduced()) {
    await sleep(150);
    [attackLine,defendLine,finger,nailBed,burst,word].forEach((el) => el?.remove());
    return;
  }
  boardNudge(ctx.boardWrap, 1.5);
  const flyX = event.defenderPlayer === ctx.viewPlayer ? -105 : 105;
  const fingerA = finger.animate([
    ...
    { transformOrigin:`${target.x}px ${target.y}px`, transform:`translate(${flyX*.28}px,-42px) rotate(${flyX>0?15:-15}deg) scale(1.12)`, opacity:1, offset:.34 },
    { transformOrigin:`${target.x}px ${target.y}px`, transform:`translate(${flyX}px,-138px) rotate(${flyX>0?62:-62}deg) scale(.74)`, opacity:0 },
  ], { duration:570, easing:"cubic-bezier(.16,.82,.24,1)" });
  const bedA = nailBed.animate([
    { transformOrigin:`${target.x}px ${target.y}px`, transform:"translate(0,0) rotate(0deg) scale(1)",opacity:1 },
    { transformOrigin:`${target.x}px ${target.y}px`, transform:`translate(${-flyX*.42}px,-88px) rotate(${flyX>0?-105:105}deg) scale(1.12)`,opacity:0 },
  ], { duration:440, delay:45, easing:"ease-out" });
  const burstA = burst.animate([
    { transformOrigin:`${target.x}px ${target.y}px`, transform:"scale(.4)",opacity:.8 },
    { transformOrigin:`${target.x}px ${target.y}px`, transform:"scale(2.5)",opacity:0 },
  ], {duration:330,easing:"ease-out"});
  const defA = defendLine?.animate([{opacity:1},{transform:`translate(${flyX*.28}px,-62px) rotate(${flyX>0?13:-13}deg)`,opacity:0}],{duration:420,easing:"ease-out"});
  const attackA = attackLine.animate([
    { opacity:1, strokeDasharray:"1 0" },
    { opacity:1, strokeDasharray:"26 13", offset:.22 },
    { transform:`translate(${flyX*.1}px,34px) rotate(${flyX>0?-5:5}deg)`, opacity:0, strokeDasharray:"15 23" },
  ], { duration:520, delay:75, easing:"ease-in" });
  const pieces = [
    ...capturePieces(attackerPts,ctx,"fx-capture-piece attacker",1),
    ...capturePieces(defenderPts,ctx,"fx-capture-piece defender",.8),
  ];
  const wordA = word.animate([
    { transform:"translate(-50%,-50%) rotate(-8deg) scale(.2)",opacity:0 },
    { transform:"translate(-50%,-50%) rotate(5deg) scale(1.2)",opacity:1,offset:.5 },
    { transform:"translate(-50%,-50%) rotate(-3deg) scale(1)",opacity:1,offset:.76 },
    { transform:"translate(-50%,-50%) rotate(-3deg) scale(.92)",opacity:0 },
  ], { duration:620,easing:"cubic-bezier(.2,.9,.25,1)" });
  await Promise.allSettled([fingerA.finished,bedA.finished,burstA.finished,defA?.finished,attackA.finished,wordA.finished,...pieces].filter(Boolean));
  [attackLine,defendLine,finger,nailBed,burst,word].forEach((el) => el?.remove());
}
