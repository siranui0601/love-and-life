...
function preset() { placements = []; nextId = 1; selectedGimmick = null; editingPlacement = null; currentDrop = null; deletePreview = false; for (const p of PRESET) addPlacement(p.gid, p.target, true); log('おすすめ配置を適用。味方10番中心に反撃ラインを作成。'); render(); }
...
function handleFieldPointerUp(e) {
  ...
  if (editingPlacement) {
    ...
    if (target?.ok && target.target.type === 'field') {
      ...
      log(`${g.name}はそこには再配置できない。コート上の光る範囲に置いてください。`);
    }
    editingPlacement = null;
    currentDrop = null;
    deletePreview = false;
...
function hints(g) {
  ...
  if (g.allowed.includes('field')) {
    ...
    labelBox(editingPlacement ? '動かして再配置 / コート外で撤去' : 'コート上をタップ/なぞって設置', FIELD.w / 2, FIELD.topGoalY + 150, '#e8ff66');
...
function updateDebug() { els.debug.textContent = [`state: ${els.state.textContent}`, `time: ${elapsed.toFixed(2)}s`, `ball: x=${ball.x.toFixed(1)}, y=${ball.y.toFixed(1)}`, `velocity: ${mag(ball).toFixed(3)} (${ball.vx.toFixed(2)}, ${ball.vy.toFixed(2)})`, `field: ${FIELD.w}x${FIELD.h}`, `goal x: ${goalLeft.toFixed(0)}-${goalRight.toFixed(0)}`, `cost: ${cost()}/${COST_LIMIT}`, `placements: ${placements.length}`, `selected: ${activeGimmick()?.name ?? 'なし'}`, `editing: ${editingPlacement ? gimmick(editingPlacement.gid).name : 'なし'}`, `speed: ${SPEEDS[speedIndex]}x`, `engine: built-in deterministic 2D`, `hitbox: ${hitbox ? 'ON' : 'OFF'}`].join('\n'); }
