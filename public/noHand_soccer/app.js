...
const EXACT_JA_NAMES = new Map(Object.entries({
  alien: '宇宙人',
  'alien monster': 'エイリアンモンスター',
  'anatomical heart': '心臓',
  'angry face': '怒った顔',
  'angry face with horns': '怒った角つき顔',
  'anguished face': '苦悶の顔',
  'anxious face with sweat': '汗をかいた不安な顔',
  'astonished face': '驚いた顔',
  'beaming face with smiling eyes': 'にこにこ笑顔',
  'confounded face': '困惑した顔',
  'confused face': '混乱した顔',
  'crying face': '泣き顔',
  'disappointed face': 'がっかりした顔',
  'dizzy face': '目を回した顔',
  'exploding head': '頭爆発',
  'expressionless face': '無表情',
  'face savoring food': 'おいしい顔',
  'face screaming in fear': '恐怖で叫ぶ顔',
  'face vomiting': '嘔吐する顔',
  'face with medical mask': 'マスク顔',
  'face with open mouth': '口を開けた顔',
  'face with steam from nose': '鼻息荒い顔',
  'face with tears of joy': '笑い泣き',
  'face with thermometer': '体温計の顔',
  'flushed face': '赤面した顔',
  'grinning face': 'にやり顔',
  'hot face': '暑い顔',
  'cold face': '寒い顔',
  'hundred points': '百点',
  rocket: 'ロケット', leg: '脚', foot: '足', brain: '脳', bone: '骨', eyes: '目', eye: '目', ear: '耳', nose: '鼻', mouth: '口', tooth: '歯', tongue: '舌',
  magnet: '磁石', dagger: '短剣', shield: '盾', sword: '剣', bomb: '爆弾', fire: '炎', ice: '氷', snowflake: '雪の結晶', cyclone: '渦巻き', tornado: '竜巻', cloud: '雲', wind: '風',
  'soccer ball': 'サッカーボール', 'goal net': 'ゴールネット', trophy: 'トロフィー', medal: 'メダル',
  'flag Japan': '日本国旗', 'Japanese dolls': 'ひな人形', 'Japanese castle': '日本の城', 'Japanese post office': '日本の郵便局',
  'person running': '走る人', 'running shoe': 'ランニングシューズ', kangaroo: 'カンガルー', motorway: '高速道路', rail: 'レール', train: '電車', car: '車', bus: 'バス', bicycle: '自転車', airplane: '飛行機',
}));
...
const TOKEN_JA = new Map(Object.entries({
  ...
  food: '食べ物', rice: '米', bread: 'パン', meat: '肉', egg: '卵', curry: 'カレー', ramen: 'ラーメン', sushi: '寿司', pizza: 'ピザ', hamburger: 'ハンバーガー', cake: 'ケーキ', coffee: 'コーヒー', tea: 'お茶', beer: 'ビール',
  ...
  ball: 'ボール', soccer: 'サッカー', goal: 'ゴール', net: 'ネット', trophy: 'トロフィー', medal: 'メダル', running: '走る', shoe: '靴', shoes: '靴',
  car: '車', bus: 'バス', train: '電車', rail: 'レール', airplane: '飛行機', plane: '飛行機', rocket: 'ロケット', ship: '船', boat: 'ボート', bicycle: '自転車', motorway: '高速道路', road: '道路',
  flag: '旗', japan: '日本', japanese: '日本', red: '赤', blue: '青', green: '緑', yellow: '黄', white: '白', black: '黒', circle: '丸', square: '四角', triangle: '三角', arrow: '矢印',
}));
...
function findFieldPlacementAt(point) {
  ...
  return fieldPlacements.find((pl) => { const size = ZONE[pl.gid] ?? { w: 180, h: 110 }; const z = { x: pl.target.x - size.w / 2, y: pl.target.y - size.h / 2, w: size.w, h: size.h }; return inside(point, z); }) ?? null;
}
...
function moveBall(dt) {
  const low = has('iceBall', (p) => p.target.type === 'ball'); const iced = zones('lane').some((z) => inside(ball, z)); const drag = iced ? 0.992 : low ? 0.988 : 0.978;
...
function tokenTranslateName(name) {
  const exact = EXACT_JA_NAMES.get(name.toLowerCase());
  if (exact) return exact;
  return name.toLowerCase().split(/[^a-z0-9]+/).filter(Boolean).map((token) => TOKEN_JA.get(token) ?? token).filter(Boolean).join('');
}
function polishJaName(item) {
  const name = String(item.name || '').trim();
  const rawJa = String(item.jaName || '').trim();
  const exact = EXACT_JA_NAMES.get(name.toLowerCase());
  if (exact) return exact;
  if (rawJa && hasJapanese(rawJa) && !hasAsciiLetters(rawJa)) return rawJa;
  const translated = tokenTranslateName(name);
  return translated || rawJa || name || item.emoji;
}
function normalizeCatalogItem(item) {
  const name = String(item.name || item.jaName || item.emoji || '').trim();
  const jaName = polishJaName({ ...item, name });
  const aliases = [...(EXTRA_ALIASES.get(item.emoji) ?? [])];
  const searchText = [item.emoji, name, jaName, item.shopCategory, ...aliases].join(' ');
  return {
    emoji: item.emoji,
    name,
    jaName,
    ...
    searchIndex: normalizeQuery(searchText),
  };
}
...
async function loadShopCatalog() {
  ...
  try {
    ...
    emojiCatalog = raw.filter((item) => item && item.emoji).map(normalizeCatalogItem);
  } catch (err) {
    emojiCatalog = FALLBACK_EMOJIS.map(normalizeCatalogItem);
...
function renderShopGrid() {
  ...
  els.shopGrid.replaceChildren(...filtered.slice(0, 260).map((item) => {
    ...
    const card = document.createElement('button'); card.type = 'button'; card.className = `emoji-shop-card ${inCart ? 'in-cart' : ''}`; card.title = `${item.jaName} / ${item.name} / ${item.price}pt`;
    card.innerHTML = `<span class="shop-emoji">${item.emoji}</span><span class="emoji-price">${item.price}</span>`;
    card.addEventListener('click', () => toggleCart(item)); return card;
  }));
}
function toggleCart(item) {
  const index = cart.findIndex((c) => c.emoji === item.emoji);
  if (index >= 0) { cart.splice(index, 1); log(`${item.emoji} ${item.jaName}をカートから外しました。`); }
  else { cart.push(item); log(`${item.emoji} ${item.jaName}をカートへ追加。`); }
  renderEconomy();
}
function renderCart() {
  const total = cart.reduce((sum, item) => sum + item.price, 0); els.cartTotal.textContent = total; els.buyCart.disabled = !cart.length || total > points;
  ...
  els.cartList.replaceChildren(...cart.map((item) => { const chip = document.createElement('button'); chip.type = 'button'; chip.className = 'cart-chip'; chip.textContent = `${item.emoji} ${item.price}`; chip.title = item.jaName; chip.addEventListener('click', () => toggleCart(item)); return chip; }));
}
function openPurchaseModal() {
  const total = cart.reduce((sum, item) => sum + item.price, 0);
  if (!cart.length) return;
  if (total > points) return log('発想ポイントが足りません。');
  els.purchaseTotal.textContent = total;
  els.purchasePreview.replaceChildren(...cart.map((item) => { const div = document.createElement('div'); div.className = 'purchase-preview-item'; div.title = `${item.jaName} / ${item.name}`; div.innerHTML = `<span>${item.emoji}</span><small>${item.price}</small>`; return div; }));
  els.purchaseModal.classList.remove('hidden');
}
function closePurchaseModal() { els.purchaseModal.classList.add('hidden'); }
function confirmPurchase() {
  const total = cart.reduce((sum, item) => sum + item.price, 0);
  if (!cart.length) return closePurchaseModal();
  if (total > points) { closePurchaseModal(); return log('発想ポイントが足りません。'); }
  points -= total;
...
function renderGenerate() {
  els.generateSlots.replaceChildren(...[0, 1, 2].map((index) => {
    const emoji = recipeSelection[index]; const b = document.createElement('button'); b.type = 'button'; b.className = `recipe-slot ${emoji ? 'filled' : ''}`; b.innerHTML = emoji ? `<span class="slot-emoji">${emoji}</span>` : `${index + 1}`;
    if (emoji) b.addEventListener('click', () => { recipeSelection.splice(index, 1); renderEconomy(); }); return b;
  }));
  els.generateBtn.disabled = recipeSelection.length === 0;
  const cards = Array.from(owned.entries()).map(([emoji, count]) => {
    const b = document.createElement('button'); b.type = 'button'; b.className = 'generate-owned-card can-repeat'; b.disabled = !canSelectEmoji(); b.innerHTML = `<span>${emoji}</span><small>所持${count} / 投入${selectedCount(emoji)}</small>`;
    b.addEventListener('click', () => { if (!canSelectEmoji()) return; recipeSelection.push(emoji); renderEconomy(); }); return b;
  });
...
function generateFromRecipe() {
  ...
  if (found) { log(`${recipeSelection.join(' + ')} から ${found.icon} ${found.name} を確認。試合画面で装着できます。`); selectGimmick(found); }
  else { if (!experimentalRecipes.some((r) => r.key === key)) experimentalRecipes.unshift({ key, recipe: [...recipeSelection], name: `未接続レシピ ${experimentalRecipes.length + 1}` }); log(`${recipeSelection.join(' + ')} は未接続レシピとして記録。AI生成接続時の入力候補です。`); renderGeneratedRecipes(); }
}
function renderGeneratedRecipes() {
  ...
  els.recipes.replaceChildren(...[...protoRows, ...fixedRows].map((row) => { const div = document.createElement('div'); div.className = 'recipe-item'; div.innerHTML = `<span><strong>${row.icon} ${row.name}</strong><small>${row.recipe.join(' + ')} / ${row.label}</small></span>${row.gimmick ? '<button type="button">装着</button>' : '<span>保留</span>'}`; if (row.gimmick) div.querySelector('button').addEventListener('click', () => selectGimmick(row.gimmick)); return div; }));
}

function loop(now = performance.now()) {
  const delta = Math.min((now - lastTs) / 1000, .1); lastTs = now;
  if (running && !ended) { acc += delta * SIM_SPEEDS[speedIndex]; while (acc >= 1 / 60) { step(1 / 60); acc -= 1 / 60; if (ended) break; } }
