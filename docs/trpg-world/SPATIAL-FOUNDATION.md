...
設計論を再調査し、[暮らす世界のレベルデザイン](LEVEL-DESIGN-PRINCIPLES.md)へ根拠・採用判断・現mapの批評を記録した。単なる曲線化や目隠しの追加を完成判定にしない。
...
`facility-sites.mjs` は全105 source施設をIDで配置する。Sheetの行順を変えても施設座標が入れ替わらない。未知/欠落/重複の施設IDは生成時に拒否する。既存座標のID化であり、この変更自体が街区再配置を済ませたことにはならない。
...
村/王都/交易都市の43棟には正面壁と実戸口を追加した。住宅1.8m、店舗等3.2m、荷扱い4mを仮寸法とし、描画・衝突・知覚を共通化する。粗い屋外gridが見落とす狭い戸口は、実入口の接続点を使って出入りし、NPCはその先で街路へ合流する。10店舗/宿には外壁の看板がある。民家の住人や未来の大型店を看板で公開しない。
