...
import {NEEDS, collapse, advanceNeeds, advanceRescue,publicCollapse} from './survival.js';
...
function chooseGoal(state,content,npc,template) {
  ...
  const appointment=(template.appointments||[]).find(a=>state.time>=a.startsAt&&state.time<a.endsAt&&!(npc.displacementKind==='eviction'&&content.occupancyClaims?.some(c=>c.id===npc.displacementCause&&c.facilityId===a.siteId)));
...
function advanceNpcs(state,content,gameDelta) {
  ...
  for (const npc of values(state.npcs)) {
    ...
    if(npc.rescueAssignment||npc.causalAssignment||npc.entrapment||npc.aftermathAssignment||npc.reviewAssignment||npc.institutionalAssignment||npc.transportAssignment||npc.custodyAssignment||npc.captive||npc.operationAssignment||npc.operationMember||npc.relocationAssignment||npc.dutyAssignment||npc.detention?.status==='held'||npc.care?.status==='injured'||npc.companionOf)continue;
    if(advanceSocialPlan(state,content,npc,gameDelta))continue;
    const original=idx.npcs.get(npc.id);if(!original)continue;let template=npc.displacedHome?{...original,home:npc.displacedHome,work:npc.displacementCause&&npc.displacementKind!=='eviction'?original.work:npc.displacedHome}:{...original};
...
export function projectWorld(state,content) {
  ...
  const cleanObject=o=>({id:o.id,name:o.eventId&&!knownIds.has(o.eventId)?'気になる現場':o.name,kind:o.kind,position:clone(o.position),asset:o.asset,rotation:o.rotation || 0,scale:o.scale || 1,interior:o.interior,signage:o.signage,crafting:o.crafting,buildingPosition:o.buildingPosition,width:o.width,depth:o.depth,height:o.height});
  ...
  const nearbyNpcs=values(state.npcs).filter(n=>n.region===p.region&&!n.travel&&!n.entrapment&&distance(n.position,p.position)<90&&hasLineOfSight(region,p.position,n.position)).map(n=>({id:n.id,name:idx.npcs.get(n.id)?.name,role:idx.npcs.get(n.id)?.publicRole||'住民',position:clone(n.position),activity:n.activity,hp:n.hp,accompanyingPlayer:n.companionOf==='player',condition:n.hp>0&&n.injury&&['leg','burn','crush','poison'].includes(n.injury.kind)?{kind:n.injury.kind,treated:n.injury.treated}:undefined,heading:n.path?.length?Math.atan2(n.path[0][0]-n.position[0],n.path[0][2]-n.position[2]):0}));
  ...
  const projected={schemaVersion:2,simulationTime:state.simulationTime,time:state.time,day:Math.floor(state.time/DAY)+1,clock:`${String(Math.floor(hour(state))).padStart(2,'0')}:${String(Math.floor(state.time/60)%60).padStart(2,'0')}`,
  ...
  if(projected.player.collapse)projected.player.collapse=publicCollapse(p.collapse);
  projected.perception=playerPerception(state,content,projected);delete projected.player.observedPlaces;delete projected.player.observedExits;delete projected.player.observedObstacles;delete projected.player.siteSearches;return projected;
}
