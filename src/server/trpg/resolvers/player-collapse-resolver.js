...
export const PLAYER_COLLAPSE_RESCUE_VERSION = "player-incapacitation-rescue-v7";
...
const COLLAPSE_CAUSE_LABELS = Object.freeze({ hunger: "空腹", fatigue: "疲労", battle_defeat: "戦闘での負傷" });

function optionalId(value) { const normalized = String(value ?? "").trim(); return normalized || null; }
function finiteMinute(value, fallback = 0) { const number = Number(value); return Number.isFinite(number) ? Math.max(0, Math.round(number)) : fallback; }
...
function sourceValue(source, id) { if (!source) return null; if (source instanceof Map) return source.get(id) ?? null; return source[id] ?? null; }
function normalizedIdSet(value) { if (value instanceof Set) return new Set([...value].map(String)); if (Array.isArray(value)) return new Set(value.map(String)); return new Set(); }
...
export function buildCollapseRescueCandidates({ presentNpcIds = [], companionNpcIds = [], npcStates = null, npcDefinitions = null, reachableNpcIds = [], location = null, facilityId = null, fallbackWakeFacilityId = null } = {}) {
  ...
  return [...candidateIds].sort((left, right) => left.localeCompare(right, "ja")).map((id) => {
    ...
    return {
      id, name: definition.name ?? state.name ?? id,
      companion: companion.has(id), present: present.has(id), canReach: reachable.has(id) && !departed,
      alive: !dead, dead, missing, detained, departed, incapacitated, hostile,
      ...
      wakeFacilityId: optionalId(state.wakeFacilityId) ?? stateFacilityId ?? optionalId(fallbackWakeFacilityId) ?? (present.has(id) ? optionalId(facilityId) : null),
      travelMinutes: finiteMinute(state.rescueTravelMinutes ?? state.travelMinutes ?? 0, 0),
      knowledge, playerTrust: Number(state.playerTrust ?? state.trustToPlayer ?? 0), rescueCost: Number(state.rescueCost ?? definition.rescueCost ?? 0),
...
export function selectCollapseRescuer(incident, candidates = []) {
  ...
  return candidates.filter(eligibleCandidate).map((candidate) => ({ candidate, score: candidateScore(candidate, incident) }))
    .sort((left, right) => right.score - left.score || optionalId(left.candidate.id).localeCompare(optionalId(right.candidate.id), "ja"))[0]?.candidate ?? null;
}

export function planIncapacitationRescue(incident, {
  candidates = [], fallbackRescuerId = "SYSTEM_LOCAL_AID",
  destination = null,
  fallbackWakeLocation = null, fallbackWakeFacilityId = null,
  fallbackDiscoveryMinutes = DEFAULT_FALLBACK_DISCOVERY_MINUTES,
  fallbackEvacuationMinutes = DEFAULT_FALLBACK_EVACUATION_MINUTES,
  rescueArrivalMinutes = null, evacuationMinutes = null,
  ...
  const wakeLocation = optionalId(destination?.location)
    ?? optionalId(fallbackWakeLocation)
    ?? optionalId(rescuer?.wakeLocation)
    ?? optionalId(rescuer?.location)
    ?? incident.location;
  const wakeFacilityId = optionalId(destination?.facilityId)
    ?? optionalId(fallbackWakeFacilityId)
    ?? optionalId(rescuer?.wakeFacilityId)
    ?? optionalId(rescuer?.facilityId)
    ?? incident.facilityId;
  const explicitArrival = rescueArrivalMinutes != null ? finiteMinute(rescueArrivalMinutes, 0) : null;
  const discoveryDelayMinutes = explicitArrival != null
    ? explicitArrival
    : usedFallback
      ? finiteMinute(fallbackDiscoveryMinutes, DEFAULT_FALLBACK_DISCOVERY_MINUTES)
      : 0;
  const explicitEvacuation = evacuationMinutes != null ? finiteMinute(evacuationMinutes, 0) : null;
  const evacuationDuration = explicitEvacuation != null
    ? explicitEvacuation
    : rescuer
  ...
  return {
    rescuer, rescuerId: optionalId(rescuer?.id) ?? optionalId(fallbackRescuerId), usedFallback,
    wakeLocation, wakeFacilityId,
    discoveryDelayMinutes,
    evacuationMinutes: evacuationDuration,
    treatmentRecoveryMinutes: recoveryMinutes,
    elapsedMinutes: discoveryDelayMinutes + evacuationDuration + recoveryMinutes,
  };
}

export function prepareCollapseCommand(player, commandType, context = {}) {
  const opened = openPlayerCollapseIncident(player, { minute: context.minute, location: context.location, facilityId: context.facilityId });
  const block = playerCollapseCommandBlock(player, commandType);
  return { version: PLAYER_COLLAPSE_RESCUE_VERSION, opened: opened.opened, incident: opened.incident ?? block.incident ?? null, blocked: block.blocked, code: block.code, commandType: block.commandType, causes: block.causes ?? [] };
}

export function collapseRescueView(player, { facilityName = null, fallbackPlaceLabel = "その場" } = {}) {
  const incident = player?.needs?.activeCollapse;
  if (!incident || incident.status !== "pending_rescue") return null;
  const causes = (Array.isArray(incident.causes) ? incident.causes : []).map((cause) => COLLAPSE_CAUSE_LABELS[cause] ?? String(cause)).filter(Boolean);
  const placeLabel = String(facilityName ?? "").trim() || fallbackPlaceLabel;
  const causeLabel = causes.length ? causes.join("と") : "体調不良";
  return {
    version: PLAYER_COLLAPSE_RESCUE_VERSION, active: true, incidentId: incident.id, status: incident.status,
    title: "力尽きて倒れた", narrative: `${placeLabel}で${causeLabel}のため動けなくなった。周囲の誰かが気づき、救助へ動いている。`,
    causes, collapsedAtMinute: Number(incident.atMinute ?? 0), location: optionalId(incident.location), facilityId: optionalId(incident.facilityId),
    command: { type: PLAYER_COLLAPSE_RESCUE_COMMAND, label: "救助を受け、目を覚ます" },
    uiLock: { choices: [], movement: [], stock: [], saleQuotes: [], loans: [], rewards: [], learnableSkills: [], battleCommands: [] },
  };
}
...
export function applyCollapseRescueView(gameView, player, options = {}) {
  ...
  return {
    ...source,
    scene: { ...(source.scene && typeof source.scene === "object" ? source.scene : {}), title: rescueView.title, narrative: rescueView.narrative, collapseRescue: rescueView },
    collapseRescue: rescueView, choices: rescueView.uiLock.choices, movement: rescueView.uiLock.movement,
    shop: { ...shop, available: false, stock: [], saleQuotes: [], loans: [], rewards: [] },
...
export function resolveCollapseRescue(player, {
  minute = 0, location = null, facilityId = null, candidates = [],
  fallbackRescuerId = "SYSTEM_LOCAL_AID", destination = null,
  fallbackWakeLocation = null, fallbackWakeFacilityId = null,
  wakeDelayMinutes = DEFAULT_WAKE_DELAY_MINUTES,
  fallbackDiscoveryMinutes = 0, fallbackEvacuationMinutes = 0,
  rescueArrivalMinutes = null, evacuationMinutes = null,
  hungerAfter = 70, fatigueAfter = 70,
} = {}) {
  const opened = openPlayerCollapseIncident(player, { minute, location, facilityId });
  const incident = opened.incident;
  if (!incident || incident.status !== "pending_rescue") return { completed: false, incident: null, rescuer: null, usedFallback: false };
  const plan = planIncapacitationRescue(incident, {
    candidates, fallbackRescuerId, destination, fallbackWakeLocation, fallbackWakeFacilityId,
    fallbackDiscoveryMinutes, fallbackEvacuationMinutes, rescueArrivalMinutes, evacuationMinutes,
    treatmentRecoveryMinutes: wakeDelayMinutes,
  });
  const rescueMinute = Math.max(incident.atMinute, finiteMinute(minute, incident.atMinute)) + plan.elapsedMinutes;
  const result = completePlayerCollapseRescue(player, { minute: rescueMinute, rescuerId: plan.rescuerId, wakeLocation: plan.wakeLocation, wakeFacilityId: plan.wakeFacilityId, hungerAfter, fatigueAfter });
