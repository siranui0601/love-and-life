...
} = world.T19_CORE_INTERNALS;
const {
  completeStep,
  recordDiscovery,
  evaluateBattleObjectives,
  applyResolutionEffect,
} = world.T19_WORLD_INTERNALS;
...
export function applyAuthoredMissionFlowAction(runtime, action, result) {
  ...
  if (kind === "opening") {
    ...
    result.summary ??= "黒嶺侵攻の最初の因果線を確定した。";
  ...
  if (kind === "intervention") {
    ...
    result.summary = `${intervention?.summary ?? "黒嶺前衛を森突破前に停止した。"} ${objectiveSummary ?? ""}`.trim();
  ...
  if (changed) {
    ...
    runtime.playerState.history.push({
      type: "AUTHORED_MISSION_FLOW_T19_UPDATED",
      minute,
...
export const T19_RUNTIME_INTERNALS = F({
  available,
  step,
  sourceAt,
  leadById,
