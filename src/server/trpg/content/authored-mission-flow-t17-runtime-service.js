...
import * as world from "./authored-mission-flow-t17-runtime-world.js";

export * from "./authored-mission-flow-t17-runtime-world.js";
...
} = world.T17_RUNTIME_WORLD_INTERNALS;

export function applyAuthoredMissionFlowAction(runtime, action, result) {
  let changed = core.applyAuthoredMissionFlowAction(runtime, action, result);
  ...
  if (kind === "opening") {
    ...
    result.summary ??= "第二召喚の最初の因果線を確定した。";
  ...
  if (kind === "intervention") {
    ...
    evaluateBattleObjectives(runtime, {
      won: true,
      rounds: 0,
      encounterId: `PRE-RITUAL:${flow.interventionChoiceId}`,
    }, result);
    const objectiveSummary = result.summary;
    result.summary = `${intervention?.summary ?? "第二召喚を戦闘前に停止した。"} ${objectiveSummary ?? ""}`.trim();
  ...
  if (changed) {
    ...
    runtime.playerState.history.push({
      type: "AUTHORED_MISSION_FLOW_T17_UPDATED",
      minute,
...
export function suppressGenericAuthoredMissionAction(runtime, action) {
  if (action?.missionId !== P.missionId) {
    return core.suppressGenericAuthoredMissionAction(runtime, action);
  }
  ...
  if (current?.id === P.battle.stepId) {
    ...
    return day < P.battle.preRitualDeadlineDay;
  }
  return false;
}

export function applyAuthoredMissionFlowCatalogOverrides(catalog) {
  const updated = core.applyAuthoredMissionFlowCatalogOverrides(catalog);
  ...
  if (battle) {
    battle.encounterId = P.battle.encounterId;
...
export function authoredMissionFlowGuidance(runtime) {
  if (!available(runtime)) return core.authoredMissionFlowGuidance(runtime);
  const flow = ensure17(runtime);
  const current = step(runtime);
