import { T19_BLACKRIDGE_INVASION_PACK as P } from "./t19-blackridge-invasion.js";

const groups = P.investigation.requiredEvidenceGroups;

export function evaluateT19EvidenceProfile(openingChoiceId, selectedEvidenceIds) {
  ...
  return P.resolution.choices.filter((route) => {
    ...
    const openingSupport = openingMatches ? Number(readiness.openingSupport ?? 0) : 0;
...
export function auditT19EvidenceProfiles() {
  ...
  for (const openingChoiceId of choices) {
    for (let profile = 0; profile < 3 ** groups.length; profile += 1) {
      ...
      const ready = evaluateT19EvidenceProfile(openingChoiceId, evidenceIds);
      results.profiles += 1;
