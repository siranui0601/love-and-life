...
export function applyCanonicalWorldModelExtensions(model) {
  ...
  if (!model.facilityById[FACILITY.id]) {
    const facility = { ...FACILITY, relatedNpcIds: [...FACILITY.relatedNpcIds], relatedTroubleIds: [...FACILITY.relatedTroubleIds] };
  ...
  model.diagnostics.push({
    ...
    severity: "info",
    message: "ライブ正本の追加施設1件・NPC1件をfixture監査後に適用した。",
    details: { facilities: [FACILITY.id], npcs: [NPC.id] },
  });
  return model;
}

export const CANONICAL_WORLD_MODEL_EXTENSION_VERSION = "virtue-route-v2-world-2026-08-16";
