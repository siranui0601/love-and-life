...
const ORIGIN_MAIN_ASSET_ROLE_MAP = {
  炎球: "projectile",
  火: "projectile",
  雷: "weather",
  プラズマ: "energy",
...
const ORIGIN_ROLE_ALLOWED_PATTERNS = {
  projectile: [
    "vertical_circle_barrage",
    "multi_phase_barrage",
    "far_projectile_impact",
...
function getMainAssetRole(mainAsset) {
...
function choosePatternFromDirectionTags(directionTags, mainAsset, seed = "") {
  ...
  const roleAllowed = ORIGIN_ROLE_ALLOWED_PATTERNS[role] || ORIGIN_MAGIC_CIRCLE_PATTERN_POOL;

  const tagFiltered = uniqueArray(tagPatterns)
    .filter((pattern) => ORIGIN_MAGIC_CIRCLE_PATTERN_POOL.includes(pattern))
    .filter((pattern) => roleAllowed.includes(pattern));

  const pool = tagFiltered.length > 0
    ? tagFiltered
    : roleAllowed.filter((pattern) => ORIGIN_MAGIC_CIRCLE_PATTERN_POOL.includes(pattern));

  return pickByStableHash(
    pool.length ? pool : ORIGIN_MAGIC_CIRCLE_PATTERN_POOL,
    seed,
    `rolePattern:${mainAsset}:${role}:${(directionTags || []).join("|")}`
  );
}
...
function finalizeVisualObjectDefaults(obj) {
  ...
  // 魔法陣・ゲート・鳥居・ドラゴン・翼・戦車などは回さない
  if (
    ORIGIN_NO_ROTATE_FILE_NAMES.has(assetFileName) ||
    ORIGIN_MAGIC_CIRCLE_FILE_NAMES.has(assetFileName)
  ) {
  ...
  // 正面を向くべき素材
  if (
    ORIGIN_FACE_ENEMY_FILE_NAMES.has(assetFileName) ||
    ORIGIN_MAGIC_CIRCLE_FILE_NAMES.has(assetFileName)
  ) {
    obj.faceEnemy = true;
  }
  ...
  if (ORIGIN_MAGIC_CIRCLE_FILE_NAMES.has(assetFileName)) {
    obj.faceEnemy = true;
    obj.rotation.shouldRotate = false;
    obj.rotationOffset = obj.rotationOffset || {
      x: Math.PI / 2,
      y: 0,
      z: 0,
    };
  ...
  // 複数個なのに none は完全重なりしやすいので避ける
  if ((Number(obj.objectCount) || 1) > 1 && (!obj.spawnSpreadPattern || obj.spawnSpreadPattern === "none")) {
    obj.spawnSpreadPattern = "circle";
  }

  // 投射物は進行方向を向け、衝突したら消える
  const moveDuration = Number(obj.movement?.moveDurationSeconds || 0);
  const targetPosition = String(obj.movement?.targetPosition || "");

  if (
    ORIGIN_PROJECTILE_FACE_TARGET_FILE_NAMES.has(assetFileName) &&
    moveDuration > 0 &&
    ["enemy_position", "above_enemy"].includes(targetPosition)
  ) {
    obj.faceMovementDirection = true;
    obj.movement.despawnOnImpact = true;
    obj.lifeTimeSeconds = Math.min(
      Number(obj.lifeTimeSeconds || 8),
      moveDuration + 0.22
    );
  }

  // 雷は物体回転させない。地面まで届く専用エフェクトとして扱う
  if (assetFileName === "lightning") {
    obj.rotation.shouldRotate = false;
    obj.lightningStyle = obj.lightningStyle || pick([
      "pillar",
      "storm",
      "road",
      "slash",
...
function buildOpeningSequence({ mainAsset, profile, circleAsset, pattern }) {
  const role = getMainAssetRole(mainAsset);
  const timed = [];
  const scene = [];

  const pushTimed = (time, objects) => {
    timed.push(makeTimedEffect(time, Array.isArray(objects) ? objects : [objects]));
  };

  const pushScene = (time, type, options = {}) => {
    scene.push(makeSceneEffect(time, type, options));
  };

  const openingType = pickByStableHash(
    [
      "front_vertical_circle",
      "ground_ritual_circle",
      "behind_symbol",
      "enemy_sky_omen",
      "silent_flash",
      "gate_opening",
  ...
  pushScene(0, "sky_color", {
    color: profile.sky,
    durationSeconds: 5.5,
  });

  if (openingType === "front_vertical_circle") {
    pushTimed(0, [
      buildVerticalCircleObject("origin_vertical_circle", circleAsset, profile.color, 4.8),
      makeVisualObject({
        id: "origin_charge_orbit",
        ...
        exit: "rise_to_sky",
        rotate: false,
        positionOffset: { forward: 2.8, y: 2.2 },
        orbit: {
  ...
  if (openingType === "ground_ritual_circle") {
    pushTimed(0, makeVisualObject({
      id: "origin_ground_circle",
      asset: circleAsset,
      count: 1,
      position: "self_position",
      size: "large",
    ...
    pushScene(0.15, "camera_top_down", {
      durationSeconds: 0.7,
      holdSeconds: 0.55,
      y: 16,
      lookAt: "self",
  ...
  if (openingType === "behind_symbol") {
    pushTimed(0, [
      makeVisualObject({
        id: "origin_behind_symbol",
        asset: role === "creature" ? mainAsset : pick(["翼", "天使翼", "悪魔翼", "オーラ"]),
        count: 1,
        ...
        rotate: false,
        faceEnemy: true,
        positionOffset: { forward: -2.5, y: 3.2 },
      }),
      makeVisualObject({
        id: "origin_small_charge",
        asset: "光球",
        count: 5,
        position: "in_front_of_self",
        size: "small",
        ...
        enter: "scale_up",
        exit: "scale_down",
        positionOffset: { forward: 3.4, y: 2.2 },
      }),
    ]);

    pushScene(0.2, "camera_move", {
      durationSeconds: 0.8,
      holdSeconds: 0.5,
      right: 3.8,
      y: 2.8,
  ...
  if (openingType === "enemy_sky_omen") {
    pushTimed(0, makeVisualObject({
      id: "origin_enemy_sky_omen",
      asset: pick(["魔法陣9", "雲", "銀河", "月"]),
      count: 3,
      ...
      rotate: false,
      faceEnemy: true,
      positionOffset: { y: 7.5 },
      rotationOffset: { x: Math.PI / 2, y: 0, z: 0 },
    }));

    pushScene(0.1, "camera_top_down", {
      durationSeconds: 0.8,
      holdSeconds: 0.7,
      y: 20,
      lookAt: "enemy",
  ...
  if (openingType === "silent_flash") {
    pushTimed(0.35, makeVisualObject({
      id: "origin_flash_ring",
      asset: "シンプルリング",
      count: 1,
      position: "battlefield_center",
      size: "large",
    ...
    pushScene(0.05, "camera_pulse", {
      durationSeconds: 1.1,
      strength: 0.5,
    });
  }

  if (openingType === "gate_opening") {
    pushTimed(0, makeVisualObject({
      id: "origin_opening_gate",
      asset: pick(["ゲート1", "ゲート2", "円盤", "鳥居"]),
      count: 1,
      position: "in_front_of_self",
      size: "large",
      life: 5.4,
      ...
      rotate: false,
      faceEnemy: true,
      positionOffset: { forward: 3.2, y: 2.2 },
    }));
  ...
  if (openingType === "wide_orbit_charge") {
    pushTimed(0, makeVisualObject({
      id: "origin_wide_orbit_charge",
      asset: pick(["光球", "シンプルリング", "プラズマ", "雲"]),
      count: 8,
      position: "self_position",
      size: "small",
      life: 3.8,
...
function buildOriginMagicCircleEffectFromConcept(concept) {
  ...
  const opening = buildOpeningSequence({
  mainAsset,
  profile,
  circleAsset,
  pattern,
...
opening.timed.forEach((effect) => timedVisualEffects.push(effect));
opening.scene.forEach((effect) => sceneEffects.push(effect));
  ...
  if (
    ...
    add(0.8, [
      makeVisualObject({
        ...
        rotate: false,
        faceEnemy: true,
        positionOffset: { forward: 2.5, y: 2.3 },
        rotationOffset: { x: Math.PI / 2, y: 0, z: 0 },
      }),
      makeVisualObject({
        id: "chain_circle_2",
    ...
    add(2.0, makeVisualObject({
      ...
      enter: "scale_up",
      exit: "scale_down",
      positionOffset: { forward: 3.8, y: 2.4 },
    }));

    add(3.6, makeVisualObject({
      id: "chain_finish",
      asset: finishAsset,
      count: 1,
      position: "enemy_position",
  ...
  //上空円形から降り注ぐ
    ...
    family === "sky_to_ground_lightning" ||
    family === "field_lightning_rain" ||
    family === "sky_mass_rain" ||
    family === "camera_sky_execution"
  ) {
    ...
    add(1.8, makeVisualObject({
      id: "sky_rain_main",
      asset: rainAsset,
      count: mainAsset === "雷" ? 8 : 10,
      position: "above_enemy",
      size: mainAsset === "雷" ? "large" : "medium",
      life: mainAsset === "雷" ? 2.4 : 2.0,
      color: profile.color,
      spread: "circle",
      target: "enemy_position",
      duration: mainAsset === "雷" ? 0 : 0.95,
      path: mainAsset === "雷" ? "none" : "fall_from_above",
      enter: "fall_from_sky",
      exit: "scale_down",
      lightningStyle: family === "field_lightning_rain" ? "storm" : "pillar",
    }));
    ...
    family === "enemy_wide_impale" ||
    family === "ice_bullet_stab" ||
    family === "weapon_execution"
  ) {
    const weaponAsset = mainAsset;

    add(1.0, makeVisualObject({
      id: "impale_warning_ring",
      asset: "シンプルリング",
      count: 8,
      position: "enemy_position",
      size: "medium",
    ...
    add(2.0, makeVisualObject({
      id: "meteor_main",
      asset: mainAsset === "隕石" ? "隕石" : mainAsset,
      count: 1,
      position: "above_enemy",
      size: "large",
      life: 1.35,
    ...
    add(3.1, makeVisualObject({
      id: "meteor_impact_burst",
      asset: "大爆発",
      count: 1,
      position: "enemy_position",
      size: "large",
    ...
    add(1.4, makeVisualObject({
      id: "clock_frozen_rings",
      asset: "シンプルリング",
      count: 8,
      position: "enemy_position",
      size: "medium",
    ...
    add(2.8, makeVisualObject({
      id: "clock_main_execution",
      asset: mainAsset,
      count: mainAsset === "歯車時計" ? 3 : 5,
      position: "above_enemy",
      size: "medium",
      life: 2.2,
      color: profile.color,
    ...
    addScene(2.8, "camera_pulse", {
      durationSeconds: 1.0,
      strength: 0.55,
    });

    impactTimes.push(2.9, 4.4, 6.6);
    
    
    
    //月
  } else if (family === "moon_gravity") {
    add(0.9, makeVisualObject({
      id: "moon_overhead",
      asset: "月",
      count: 1,
    ...
    add(2.6, makeVisualObject({
      id: "gravity_collapse",
      asset: mainAsset === "月" ? "銀河" : mainAsset,
      count: 1,
      position: "enemy_position",
      size: "large",
      life: 4.0,
    ...
    addScene(2.6, "camera_shake", {
      durationSeconds: 1.4,
      strength: 0.5,
    });

    impactTimes.push(2.8, 4.2, 6.4);
    
    
    
    
    //ゲートから射出
  } else if (family === "portal_projectile_burst") {
    add(0.7, [
      makeVisualObject({
        id: "portal_left",
        asset: gateAsset,
        ...
        rotate: false,
        faceEnemy: true,
        positionOffset: { forward: 3.4, right: -3.2, y: 2.4 },
      }),
      makeVisualObject({
        id: "portal_right",
        asset: gateAsset,
    ...
    add(1.5, makeVisualObject({
      id: "portal_burst_main",
      asset: mainAsset,
      count: 10,
      position: "in_front_of_self",
      size: "medium",
      ...
      enter: "scale_up",
      exit: "scale_down",
      positionOffset: { forward: 4.2, y: 2.4 },
    }));

    add(3.2, makeVisualObject({
      id: "portal_burst_finish",
      asset: finishAsset,
      count: 1,
      position: "enemy_position",
      size: "large",
    ...
    addScene(1.5, "camera_shake", {
      durationSeconds: 1.1,
      strength: 0.45,
    });

    impactTimes.push(2.2, 3.4, 5.8);
    
    
    
    
    //戦車包囲
  } else if (family === "tank_siege") {
    const siegeAsset = mainAsset === "戦車" ? "戦車" : mainAsset;
    ...
    addScene(3.0, "camera_shake", {
      durationSeconds: 1.6,
      strength: 0.75,
    });

    impactTimes.push(3.1, 4.5, 6.8);
