...
const LABELS = { launcher: "発射", bumper: "反発", fieldForce: "力場", gravityShift: "重力", split: "分裂", portal: "ワープ", rail: "レール", carrier: "運搬", flipper: "腕", phase: "透過", dragZone: "減速", timerRelease: "捕球" };
const EMOJI_HINTS = [
  [/⛩️|🚪|🚪|🛕/, "鳥居門", "門", "通過・門"],
...
function text(value, fallback = "", max = 80) { const s = String(value || fallback || "").replace(/[\r\n\t]+/g, " ").replace(/\s+/g, " ").trim(); return s.slice(0, max); }
...
function localConcept(emojis) {
  const seed = hash(emojis.join("|"));
  const hints = emojis.map(hintFor);
  const motifs = [...new Set(hints.map((h) => h.motif))].slice(0, 3);
  const has = (re) => emojis.some((e) => re.test(e));
  let motors;
  if (has(/🔨|🤛|👊|🥊/) || has(/🚀|⚡|🔥/)) motors = [m("timerRelease", "angle", .42, .34, .28, "hold"), m("launcher", "angle", .74, .4, .18, "hit")];
  else if (has(/⛩️|🚪|🛕/)) motors = [m("timerRelease", "angle", .38, .36, .25, "gate"), m("launcher", "angle", .62, .34, .18, "pass")];
  else if (has(/🛤️|🛝|🪜/)) motors = [m("rail", "angle", .52, .42, .7, "guide"), m("launcher", "angle", .4, .3, .22, "exit")];
  else if (has(/🧲|🌀|🌪️/)) motors = [m("fieldForce", pick(["radialIn", "radialOut", "tangent"], seed), .44, .52, .7, "field"), m("dragZone", "angle", .18, .38, .35, "trim")];
  else motors = [m("launcher", "angle", .62, .36, .24, "push"), m("bumper", "radialOut", .42, .3, .18, "rim")];
  const shape = shapeFor(motors);
  const visualLabel = makeLabel(hints, motors);
  return finish({ visualLabel, emojiReading: hints.map(({ emoji, role, mechanic }) => ({ emoji, role, mechanic })), visualMotifs: motifs, motionIdea: `${motifs.join("・")}でボールを受け、向いている方向へ動かす。`, body: { shape, solid: solidFor(shape, motors), size: .7, motion: "none", motionPower: .45 }, motors }, emojis);
}
...
function makeLabel(hints, motors) { const noun = motors.some((m) => m.kind === "timerRelease") ? "捕球機" : motors.some((m) => m.kind === "rail") ? "誘導路" : motors.some((m) => m.kind === "fieldForce") ? "力場" : "射出台"; return jp(`${hints[0]?.role || "絵文字"}${hints[1]?.role || ""}${noun}`.replace(/[^぀-ヿ㐀-鿿ー々]/g, ""), "絵文字装置", 18); }
function effect(motors, motifs = []) { const a = motors[0]; const b = motors[1]; if (a?.kind === "timerRelease" && b?.kind === "launcher") return `一瞬受け止め、${motifs[1] || motifs[0] || "装置"}で向いている方向へ打ち出す`; if (a?.kind === "rail") return "触れたボールをレールに沿わせる"; if (a?.kind === "fieldForce") return "範囲内のボールをゆっくり流す"; if (a?.kind === "bumper") return "触れたボールを跳ね返す"; return "触れると向いている方向へ押し出す"; }
function finish(raw, emojis) { const fallback = localConceptRaw(emojis); const motors = normalizeMotors(raw?.motors, fallback.motors); const shape = one(raw?.body?.shape, SHAPES, shapeFor(motors)); const motifs = (Array.isArray(raw?.visualMotifs) ? raw.visualMotifs : fallback.visualMotifs).map((x) => jp(x, "素材", 16)).slice(0, 4); const reading = emojis.map((emoji, i) => { const h = hintFor(emoji, i); const r = Array.isArray(raw?.emojiReading) ? raw.emojiReading[i] : null; return { emoji, role: jp(r?.role, h.role, 14), mechanic: jp(r?.mechanic, h.mechanic, 18) }; }); const visualLabel = sanitizeLabel(raw?.visualLabel, reading, motors); const result = { name: emojis.join(""), visualLabel, emojiReading: reading, visualMotifs: motifs, motionIdea: jp(raw?.motionIdea, `${motifs.join("・")}でボールを動かす。`, 70), body: { shape, solid: solidFor(shape, motors) || Boolean(raw?.body?.solid), size: num(raw?.body?.size, .7, .35, 1), motion: one(raw?.body?.motion, MOTIONS, "none"), motionPower: num(raw?.body?.motionPower, .45, 0, 1) }, motors, exit: motors.some((m) => FIELD_KINDS.has(m.kind)) ? { direction: "radialOut", minSpeed: 6.2, afterSeconds: 1.4, label: "救済" } : null };
  result.mainMotorKinds = [...new Set(motors.map((x) => x.kind))].slice(0, 3);
  result.shortEffect = effect(motors, motifs);
  result.conceptKey = [result.name, result.visualLabel, result.mainMotorKinds.join("+")].join("|");
  return result; }
function localConceptRaw(emojis) { const seed = hash(emojis.join("|fallback")); const motors = [m(pick(["launcher", "timerRelease", "bumper"], seed), "angle", .6, .36, .25, "fallback"), m("launcher", "angle", .55, .3, .2, "assist")]; return { motors, visualMotifs: emojis, visualLabel: "絵文字射出台" }; }
function sanitizeLabel(label, reading, motors) { const clean = text(label, "", 18).replace(/\p{Extended_Pictographic}/gu, "").replace(/^[・\s]+|[・\s]+$/g, ""); if (/[ぁ-んァ-ン一-龥]/.test(clean) && !/^(ゲート|装置|細工|仕掛け|射出台)$/.test(clean)) return clean; return makeLabel(reading.map((r) => ({ role: r.role })), motors); }
function prompt(emojis) { return `3絵文字から1つの装置JSONを作る。説明は短く。trigger/name/shortEffect/deviceType不要。contactはshape判定で処理する。\n出力:{"visualLabel":"固有名","emojiReading":[{"emoji":"${emojis[0]}","role":"","mechanic":""},{"emoji":"${emojis[1]}","role":"","mechanic":""},{"emoji":"${emojis[2]}","role":"","mechanic":""}],"visualMotifs":[""],"motionIdea":"1文","body":{"shape":"point|area|fan|gate|rail|carrier|arm|platform","solid":false,"size":0.7,"motion":"none|spin|slide|bob|swing|orbit|pendulum","motionPower":0.4},"motors":[{"kind":"launcher|bumper|fieldForce|gravityShift|split|portal|rail|carrier|flipper|phase|dragZone|timerRelease","direction":"angle|up|down|left|right|radialIn|radialOut|tangent|awayFromBall","power":0.6,"range":0.4,"duration":0.3,"angle":0,"count":2,"spreadAngle":34,"mode":""}]}\nvisualLabelは絵文字だけ+ゲート禁止。motorsは最大3。絵文字:${emojis.join(" ")}`; }
...
export function mountCompactNoHandSoccerRoutes(app) {
  app.post("/api/nohand-soccer/gimmick", async (req, res) => {
    ...
    try {
      const text = await genWithFallback(prompt(emojis), { generationConfig: { responseMimeType: "application/json", temperature: .9 } });
      const gimmick = finish(JSON.parse(stripJsonFence(text)), emojis);
      await log(emojis, gimmick, "gemini-compact");
      return res.json(gimmick);
    } catch (error) {
      console.warn("[noHand-soccer] compact fallback", error);
      const gimmick = { ...localConcept(emojis), source: "fallback" };
      await log(emojis, gimmick, "fallback-compact");
      return res.json(gimmick);
    }
  });
}
