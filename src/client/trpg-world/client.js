...
async function receive(data){if(data.content)content=data.content;if(data.view){seq=data.view.lastSeq??seq;if(data.view.revision<displayRevision)return;displayRevision=data.view.revision;view=data.view;await scene.update(view);const action=view.player.actionInstance||view.player.lastActionInstance;if(action&&scene.shownActionId!==action.id){scene.shownActionId=action.id;scene.animateAttack(action.targetId,action.skillId==='magic');}if(action?.resolved&&scene.shownHitId!==action.id){scene.shownHitId=action.id;if(action.result?.damage>0)scene.animateAttack(action.targetId,action.skillId==='magic',{hit:true});}renderHud();}}
...
function journal(){dialogTarget=null;openPanel('旅の手帳');const body=$('panelBody');body.append(element('p','自分が見聞きしたことだけを記録しています。','muted'));
 ...
 for(const event of view.knownEvents){const card=element('div',null,'journal-card');card.append(element('small',statusNames[event.status]||event.status),element('h3',event.name),element('p',event.description));if(Number.isFinite(event.remaining)&&['active','critical','latent'].includes(event.status))card.append(element('small',`期限まで 約${Math.ceil(event.remaining/3600)}時間 · ${event.source?.type==='seen'?'目撃':'伝聞・調査'}`));body.append(card);}
